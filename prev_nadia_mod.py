...
def step_one(knots, basis, m, n, f, fixed_left_value=None, fixed_right_value=None):
    ...
    A = np.concatenate(
        [
            ...
            prev_nadia.build_P_matrix(temp_basis, knots, m),
...
def exchange(i, t_star, d_star, f, S, basis, knots, n):
    ...
    if knot_index is not None:
        ...
        left_sign = np.sign(prev_nadia.deviation(f, S, left_i, left_pt))
        right_sign = np.sign(prev_nadia.deviation(f, S, right_i, right_pt))
    ...
    # Normal point: look only in current interval
        ...
        if (
            left_pt is not None
            and np.sign(prev_nadia.deviation(f, S, i, left_pt)) == t_star_sign
        ):
            t_tilde = left_pt

        elif (
            right_pt is not None
            and np.sign(prev_nadia.deviation(f, S, i, right_pt)) == t_star_sign
        ):
            t_tilde = right_pt
        ...
        if t_tilde is None and n == 1:
            ...
            if (
                end_pt is not None
                and np.sign(prev_nadia.deviation(f, S, i, end_pt)) == t_star_sign
            ):
                t_tilde = end_pt
        if t_tilde is None:
    ...
    basis_deviations = np.array(
        [prev_nadia.deviation(f, S, i, t) for t in basis_points]
    )
...
def check_exit_1(
    ...
    pts, signs = prev_nadia.find_alternance_points(f, S, knots, n, global_max)
...
def gra(f, knots, m, n, fixed_left_value=None, fixed_right_value=None):
    ...
    for _ in range(100):
        (max_i, t_max, d_max), (min_i, t_min, d_min), (i_star, t_star, d_star) = (
            prev_nadia.find_extrema_overall(f, S, knots, n)
        )
    ...
    _, _, (i_star, t_star, d_star) = prev_nadia.find_extrema_overall(f, S, knots, n)
