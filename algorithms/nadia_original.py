...
from .. import helper, plotting, test_functions
...
def check_exit_1(
    ...
    # Deviation function as a function of interval index and t
    def dev(i, t):
        return deviation(f, S, i, t)

    pts, signs, _ = helper.find_alternance_points(
        dev, knots, global_max=global_max, tol=ALTERNANCE_TOL
    )
...
def gra(
    ...
    for _ in range(100):
        (max_i, t_max, d_max), (min_i, t_min, d_min), (i_star, t_star, d_star) = (
            helper.find_extrema_overall(deviation, knots, f, S)
        )
    ...
    _, _, (i_star, t_star, d_star) = helper.find_extrema_overall(deviation, knots, f, S)
